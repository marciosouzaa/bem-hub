...
import { changeOrganizationPlanAction } from "@/features/billing/actions";
import { getEntitlements } from "@/features/billing/entitlements";
import { PlanChangeButton } from "@/features/billing/plan-change-button";
...
> = {
  ...
  business: {
    name: "Business",
    price: "R$ 599",
    description: "Para operações maiores com mais usuários e integrações.",
    signal: "Escala",
  },
};
...
export default async function BillingSettingsPage({
  ...
  return (
    <section className="space-y-6">
      <PageHeader
        description="Controle o plano ativo da organização enquanto o checkout definitivo não entra no MVP. A troca atualiza os limites usados pelas validações server-side."
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_320px]">
        <Card className="os-panel-glow">
          <CardContent className="p-6">
            ...
            <h2 className="mt-5 max-w-3xl text-3xl font-semibold tracking-normal">
              Libere módulos conforme a operação crescer
            </h2>
            <p className="mt-4 max-w-3xl text-base leading-7 text-muted-strong">
              Esta tela substitui temporariamente o contato comercial. Em
              produção, ela deve evoluir para checkout, faturas e histórico de
              assinatura.
            </p>
            {featureLabel ? (
              <div className="mt-5 rounded-[var(--radius-control)] border border-warning/50 bg-panel-elevated px-4 py-3 text-sm text-muted-strong">
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <InfoCard
          ...
          text="Chat e criação de assistentes consultam o plano antes de executar a operação."
        ...
        <InfoCard
          ...
          text="A troca manual é temporária para acelerar validação durante a construção do produto."
          title="Modo build"
        />
      </section>
    </section>
      ...
      >
        <CardHeader>
          <div className="flex items-center justify-between gap-3">
            ...
            {current ? (
              <span className="rounded-[8px] bg-panel px-2 py-1 text-xs text-primary">
                Atual
              </span>
            ) : null}
          </div>
        ...
        <CardContent className="flex flex-1 flex-col gap-5">
          ...
          <form action={changeOrganizationPlanAction} className="mt-auto">
            <input name="planKey" type="hidden" value={planKey} />
            <PlanChangeButton canManage={canManage} current={current} />
          </form>
        </CardContent>
      </Card>
    </MotionSurface>
...
function UsageLine({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex items-center justify-between rounded-[var(--radius-control)] border border-panel-border bg-panel-elevated px-3 py-2">
  ...
  return (
    <Card>
      <CardContent className="p-5">
        <span className="flex size-10 items-center justify-center rounded-[var(--radius-control)] bg-sidebar-active text-primary">
          {icon}
        </span>
