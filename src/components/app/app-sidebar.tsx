...
export function AppSidebar({ canManage, collapsed = false, mobile = false, onClose, onToggle, organization }: AppSidebarProps) {
  ...
  return <aside className="flex h-full min-h-0 flex-col border-r border-panel-border bg-sidebar">
    <div className={cn("flex h-20 shrink-0 items-center border-b border-panel-border/70", compact ? "justify-center px-3" : "px-5")} style={compact ? { justifyContent: "center", paddingLeft: 12, paddingRight: 12 } : undefined}>
    ...
    <div className={cn("shrink-0 space-y-3 border-t border-panel-border/70 py-4", compact ? "px-3" : "px-4")}>
      <Button asChild className={cn("w-full", compact && "px-0")} size={compact ? "icon" : "lg"} style={{ width: "100%" }} title={compact ? "Perguntar a IA" : undefined}>
        <Link aria-label="Perguntar a IA" href="/app/chat" onClick={onClose}><Sparkles className="size-4" />{!compact ? "Perguntar a IA" : null}</Link>
      </Button>
      ...
      {!mobile ? <Button aria-label={compact ? "Expandir menu" : "Recolher menu"} onClick={onToggle} size={compact ? "icon" : "sm"} style={{ width: "100%" }} title={compact ? "Expandir menu" : undefined} variant="ghost">
